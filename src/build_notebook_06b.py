...
| **Inventory-led Model** | — | The platform buys and holds inventory itself (like a retailer), so it controls pricing, stocking, and margin. Blinkit's model: 90% inventory-led. Margin benefit is Blinkit's observed ~100 bps EBITDA accretion (+~300 bps gross margin; Eternal CFO, Q2 FY26) over a full transition — and for Instamart it's an FDI/IOCC-gated option, not yet realised. |
...
| **Density** | Orders per Store per Day | How productively each dark store is used. The central economic variable in this case study: higher density spreads fixed store costs over more orders, directly improving contribution margin. Instamart: ~1,093 (Q4 FY26); Blinkit: ~1,357 (reported, closing-capacity basis); Zepto: ~2,140 (active-store basis) - the two are not strictly comparable. |
